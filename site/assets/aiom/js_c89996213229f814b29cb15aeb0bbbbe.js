...
(function (global, factory) {
    ...
    function toList$$1(value) {
        return $.isArray(value)
            ? value
            : isString$$1(value)
                ? value.split(',').map(function (value) { return toBoolean$$1(value.trim()); })
                : [value];
    }
    ...
    var boot = function (UIkit) {
        ...
        if (Observer) {
            ...
            ready$$1(function () {
                ...
                on$$1(document.documentElement, 'DOMNodeInserted', function (e) { return apply(e.target, UIkit.connect); });
                on$$1(document.documentElement, 'DOMNodeRemoved', function (e) { return apply(e.target, UIkit.disconnect); });
            });
    ...
    var stateAPI = function (UIkit) {
        ...
        UIkit.prototype._initObserver = function () {
            ...
            var attrs = ($.isArray(this.$options.attrs)
                    ? this.$options.attrs
                    : Object.keys(this.$options.props).map(function (key) { return hyphenate$$1(key); })
            );
        ...
        methods: {
            ...
            toggleElement: function toggleElement(targets, show, animate$$1) {
                ...
                var toggles, body = document.body, scroll = body.scrollTop,
                    all = function (targets) { return promise$$1.all(targets.toArray().map(function (el) { return this$1._toggleElement(el, show, animate$$1); })).then(null, function () {}); },
            ...
            toggleNow: function toggleNow(targets, show) {
                ...
                return promise$$1.all($__default(targets).toArray().map(function (el) { return this$1._toggleElement(el, show, false); })).then(null, function () {});
            },
            ...
            _toggleElement: function _toggleElement(el, show, animate$$1) {
                ...
                if (Animation$$1.inProgress(el)) {
                    return Animation$$1.cancel(el).then(function () { return this$1._toggleElement(el, show, animate$$1); });
                }
            ...
            _toggleHeight: function _toggleHeight(el, show) {
                ...
                return Transition$$1.cancel(el).then(function () {
                    ...
                    return promise$$1(function (resolve) { return requestAnimationFrame(function () {
                            ...
                            (show
                                ? Transition$$1.start(el, $.extend(this$1.initProps, {overflow: 'hidden', height: endHeight}), Math.round(this$1.duration * (1 - height / endHeight)), this$1.transition)
        ...
        UIkit.component('navbar-dropbar', {
            ...
            methods: {
                ...
                transitionTo: function transitionTo(height) {
                    ...
                    return Transition$$1.cancel(this.$el).then(function () { return Transition$$1.start(this$1.$el, {height: height}, this$1.duration); });
                }
    ...
    var Svg = function (UIkit) {
        ...
        UIkit.component('svg', {
            ...
            connected: function connected() {
                ...
                this.svg = this.getSvg().then(function (doc$$1) { return promise$$1(function (resolve, reject) { return fastdom$$1.mutate(function () {
        ...
        UIkit.component('lightbox', {
            ...
            computed: {
                ...
                toggles: function toggles() {
                    ...
                    return $$$1(this.toggle, this.$el).each(function (_, el) { return this$1.items.push({
        ...
        UIkit.mixin({
            ...
            events: {
                ...
                showitem: function showitem(e) {
                    ...
                    ajax$$1({type: 'GET', url: ("http://vimeo.com/api/oembed.json?url=" + (encodeURI(item.source))), jsonp: 'callback', dataType: 'jsonp'}).then(function (res) { return setIframe(res.width, res.height); });
...
window.onload=function(){TweenMax.to('.bg-mask',1,{opacity:0,delay:0.5,ease:Power2.easeIn});$('.bg-mask').css({"z-index":0});};$(document).ready(function(){if($('body').attr('id')==="template-project-single"){$('html').animate({scrollTop:0},1);$('body').animate({scrollTop:0},1);TweenMax.set('.nav-wrapper',{y:"-80px",opacity:0});TweenMax.set('.hero p',{y:"40px",opacity:0});TweenMax.set('.hero h1',{y:"40px",opacity:0});var heroHeight=$('.hero').outerHeight();var bgStart=$('.hero').attr('data-bgColorStart');var bgEnd=$('.hero').attr('data-bgColorEnd');$('.trans-layer').css({"background":"linear-gradient(40deg, rgba("+bgStart+",1) 0%, rgba("+bgEnd+",1) 100%)","opacity":1,"z-index":9999999,"top":0,});$('.hero').css({"opacity":0});$('.bg-mask').css({"background":"linear-gradient(40deg, rgba("+bgStart+",1) 0%, rgba("+bgEnd+",1) 100%)","z-index":999999});TweenMax.to('.trans-layer',1,{height:heroHeight,ease:Power3.easeIn,onComplete:function(){var bgImage=$('.hero').attr('data-bgImage');var bgPosition=$('.hero').attr('data-bgPosition');var bg='url('+bgImage+') center '+bgPosition+' no-repeat / cover';$('.hero').css({"background":bg});}});TweenMax.to('.hero',0.01,{opacity:1,delay:0.99,onComplete:function(){TweenMax.to('.nav-wrapper',0.5,{y:"0px",opacity:1,ease:Power1.easeOut});TweenMax.to('.hero p',0.4,{y:"-40px",opacity:1,ease:Power1.easeOut});TweenMax.to('.hero h1',0.2,{y:"-40px",opacity:1,delay:0.1,ease:Power1.easeOut});}});TweenMax.to('.trans-layer',1,{opacity:0,delay:1,onComplete:function(){TweenMax.set('.trans-layer',{height:0,zIndex:1});}});$('.trans-link a').click(function(){TweenMax.to('.hero',0.6,{opacity:0,ease:Power1.easeOut});TweenMax.to('footer, section:not(.hero)',0.6,{y:"150px",opacity:0,ease:Power1.easeOut});});}else{TweenMax.set('.nav-wrapper',{y:"-80px",opacity:0});TweenMax.to('.nav-wrapper',1,{y:"0px",opacity:1,ease:Power1.easeOut});$('.trans-link a, .trans-a').click(function(){TweenMax.to('main',0.5,{y:"100px",opacity:0,ease:Power1.easeOut});});}
$('.project-link').click(function(e){e.preventDefault();var url=$(this).attr('data-url');var bg="#2ca9e0";$('.trans-layer').css({"bottom":0,"background":bg,"opacity":1,"z-index":99999999999999999});TweenMax.to('#site-nav',0.2,{y:"-85px",ease:Power1.easeIn});TweenMax.to('.trans-layer',0.5,{height:"100%",top:"0",ease:Power1.easeOut,onComplete:function(){$('#page-wrapper').css({"opacity":0});window.location.href=url;}});});hoverInit();if($('body').attr("id")==="template-contact"||$('body').attr("id")==="template-career-single"){formInit();}
var modal=UIkit.modal("#overlay-menu");$('.burger-icon').click(function(){var links=$(".page-links ul li ");$(links).each(function(i,element){TweenMax.set(element,{y:40,opacity:0});TweenMax.to(element,0.5,{onStart:removeFadeBottom(element),y:0,delay:0.3+i*0.05,opacity:1,ease:Power2.easeOut});});function removeFadeBottom(e){$(e).removeClass("fade-from-bottom-slow");}
TweenMax.set(".social-links",{y:30,opacity:0});TweenMax.to(".social-links",0.6,{y:0,opacity:1,delay:0.6,ease:Power3.easeOut});modal.show();});$('#overlay-menu-close').click(function(){modal.hide(200);});$('.page-links a').click(function(){modal.hide(200);});$('#site-nav a, .menu li a, .trans-link a').click(function(){TweenMax.to('#site-nav',0.2,{y:"-90px",ease:Power1.easeIn});});$('form').on('click','input[type=checkbox]',function(){$(this).parent().toggleClass("checked");});$('form').on('click','input[type=radio]',function(){$('input[type=radio]').parent().removeClass("checked");$(this).parent().addClass("checked");});$('#template-careers .btn-scrolldown').click(function(){$(window).stop(true).scrollTo($('#open-positions'),{duration:600,offset:-40});});function hoverInit(){$('.project, .ways .block').hover(function(){$(this).find('.btn-view').addClass('is-hover');},function(){$(this).find('.btn-view').removeClass('is-hover');});}
window.sr=ScrollReveal({easing:'ease-out',scale:1,distance:'40px',duration:500,viewFactor:0.1});sr.reveal('.top-nav',{delay:350},0);sr.reveal('.hero h1',{duration:500},100);sr.reveal('.hero p, .btn-scrolldown',{delay:300,duration:400},0);sr.reveal('.position, .career-contact',{delay:200},50);sr.reveal('article h4, article ul, article p',{delay:200},50);sr.reveal('.sub-section h2, .sub-section .text, .name, .sub-section h5',{delay:200},0);sr.reveal('.member',{delay:200},50);sr.reveal('.item',{delay:200},50);sr.reveal('.standards h5',{delay:200},50);sr.reveal('.post',{delay:200},50);sr.reveal('.uk-legend, .uk-input, .btn-browse, .input-wrap, textarea, .btn-submit',{delay:200},50);sr.reveal('#template-home .service-item',{delay:200},50);sr.reveal('#template-about .service-item',{delay:200},80);sr.reveal('.client',{delay:200},50);sr.reveal('.info',{delay:200},50);sr.reveal('.gains',{delay:200},50);sr.reveal('.img-sr',{delay:200},50);sr.reveal('.ways h2, .ways h3, .ways p',{delay:200},50);sr.reveal('#template-services .hero p, #template-services .hero h1',{delay:250},200);sr.reveal('#template-archive .projects',{delay:150},100);sr.reveal('#template-services .model p',{delay:200},100);sr.reveal('#template-services h4, #template-services .benefits-list li, #template-services .btn-view',{delay:200},100);sr.reveal('.block',{delay:200},300);sr.reveal('.project',{delay:200},300);sr.reveal('h2',{delay:80},0);sr.reveal('h1',{delay:100},0);sr.reveal('.contact-us .h1, .contact-us p',{delay:100},100);sr.reveal('.footer-logo',{delay:100},0);sr.reveal('.img-sr',{delay:200},50);sr.reveal('#template-services .service-block',{delay:0,duration:2000,opacity:0,distance:'0px'},0);sr.reveal('#template-services .service-item',{distance:'50%',duration:700,opacity:1},0);sr.reveal('#template-services .service-item h2',{delay:600,duration:600},0);sr.reveal('#template-services .service-item .list',{delay:700,duration:600},0);});